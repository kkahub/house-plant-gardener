...
export const useNote = (name: string, code: string) => {
  ...
  const getNoteStatus = async () => {
    ...
    // 노트 상태 가져오기
    isNote.value = await hasNote(uid.value, `${guideName.value}_${guideCode.value}`)

    // 노트 내용 가져오기
    if (isNote.value) {
      noteContent.value = await getNoteContent(uid.value, `${guideName.value}_${guideCode.value}`)
  ...
  const noteSave = async (prevNote: string) => {
    ...
    if (prevNote !== noteContent.value) {
      await addNote(uid.value, `${guideName.value}_${guideCode.value}`, noteContent.value)
      alert('노트를 저장했습니다.')
    }
    isEditNote.value = false
    isNote.value = true
  ...
  const noteRemove = async () => {
    if (confirm('노트를 삭제하시겠습니까?')) {
      await removeNote(uid.value, `${guideName.value}_${guideCode.value}`)
      isNote.value = false
      isEditNote.value = false
